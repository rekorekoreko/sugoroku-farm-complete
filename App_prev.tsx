      ...
      <div className="min-h-screen bg-gradient-to-b from-green-100 to-blue-100 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">繝励Ξ繧､繝､繝ｼ蜷・/label>
              <input
                type="text"
                value={playerName}
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-100 to-blue-100 p-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-green-800 mb-2">軸 Sugoroku Farm 験</h1>
          <div className="flex justify-center items-center gap-4 text-lg">
            <span className="flex items-center gap-2">
              <Coins className="w-5 h-5 text-yellow-500" />
              {currentPlayer.coins} 繧ｳ繧､繝ｳ
            </span>
            <span>蜿守ｩｫ謨ｰ: {currentPlayer.crops_harvested}</span>
            <span>繧ｿ繝ｼ繝ｳ: {gameState.turn}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              >
                {isRolling ? '繧ｵ繧､繧ｳ繝ｭ繧呈険縺｣縺ｦ縺・∪縺・..' : '繧ｵ繧､繧ｳ繝ｭ繧呈険繧・}
              </Button>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>迴ｾ蝨ｨ縺ｮ繝槭せ</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="text-center">
                <div className="text-2xl font-bold mb-2">繝槭せ {currentPlayer.position}</div>
                {currentSquare.crop ? (
                  ...
                  <div className="text-gray-500">遨ｺ縺ｮ繝槭せ</div>
                )}
              </div>
            </CardContent>
          </Card>
